      ...
      true ->
        if
          STATUS == "KnowAndTell" ->
            ...
            list_to_atom(MSGNODE) ! {self(), hello, VALUE},
...
node_rec(MYNUM, STATUS, NUMNODES, VALUE, TRIES) ->
    receive
        ...
        {NODE_ID, hello, THEIR_VALUE} ->
            %%% Someone pinged me with their value
            io:format("I, ~p(~p): received: ~p:~p~n", [self(), STATUS, NODE_ID, THEIR_VALUE]),
            NODE_ID ! {self(), reply, VALUE},
            if
              STATUS == "KnowAndTell" ->
                if
                  THEIR_VALUE == VALUE ->
...
start(1, TOTAL) ->
    ...
    register(node1, spawn(prj_avg2, node_tell, [1, "KnowAndTell", TOTAL, STARTVALUE, 0]));
start(N, TOTAL) ->
    random:seed(now()),
    STARTVALUE = random:uniform(TOTAL * 2),
    PID = spawn(prj_avg2, node_tell, [N, "DontKnow", TOTAL, STARTVALUE, 0]),
