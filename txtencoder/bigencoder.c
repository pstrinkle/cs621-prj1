    ...
    Encoder_Context *ctx)
{
    ...
    for (i = 0; i < keyLen; i++)
    {
        /* Sadly, the rotation has to be predictable so i can
         * decode it or i can embed that information.
         */
        ofs = (key[i] - set[0])/4;
        ...
        for (i = 0; i < bytesRead; i++)
        {
            switch (task)
            {
                case Task_Encode:
                    ...
                    {
                        inputBuf[i] = encode(inputBuf[i], &context);
                    }
