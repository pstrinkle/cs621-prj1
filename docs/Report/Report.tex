...
We implemented three sets of map/reducers for this project.  The first set determines the max/min value of the file.  The mapper finds the max/min of the set of numbers it is give and returns each as a key/value pair.  The reduce then finds the max/min out of all these key value pairs.  Since we are able to send multiple rows to the mapper we can find the max/min in one map reduce step.  

The second set of map/reducers we implemented calculated the average of the file.  The mapper calculated the average and number of doubles that made up the average for the set of numbers it is given.  It return these values as a pair to the reduce which then calculated the average of the file from them.   
